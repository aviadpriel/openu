 ...
 * Course 20594
 * Semseter 2016b
 * Student: Stas Seldin (ID 311950943)
 */
...
Bool isValidJobSyntax(char *str) {
	if(!str || str[0] != '%') {
		return false;
	}

	char *jobNumStr = str + 1;
	int len = strlen(jobNumStr);
...
int runCommand(struct job newJob, struct jobSet * jobList, 
    ...
    if (!strcmp(newJob.progs[0].argv[0], "exit")) {
    	...
    	if(isValidJobSyntax(newJob.progs[0].argv[1]) == false) {
    		return 1;
    	}
